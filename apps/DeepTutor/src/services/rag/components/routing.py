...
class DocumentType(Enum):
    ...
    SPREADSHEET = "spreadsheet"  # Excel (extract to text)
...
class FileTypeRouter:
    ...
    @classmethod
    async def read_spreadsheet_file(cls, file_path: str) -> str:
        ext = Path(file_path).suffix.lower()
        max_rows = int((os.getenv("KB_SPREADSHEET_MAX_ROWS") or "500").strip() or "500")
        max_cols = int((os.getenv("KB_SPREADSHEET_MAX_COLS") or "50").strip() or "50")
        if max_rows < 1:
            max_rows = 1
        if max_cols < 1:
            max_cols = 1
                ...
                for r in range(1, rmax + 1):
                    ...
                    for c in range(1, cmax + 1):
                        ...
                        if v is None:
                            ...
                            s = str(v)
                            row_vals.append(s.replace("\t", " ").replace("\r", " ").replace("\n", " ").strip())
        ...
        def _xls():
            ...
            for sheet in book.sheets():
                ...
                rmax = min(max_rows, sheet.nrows or 0)
                cmax = min(max_cols, sheet.ncols or 0)
                for r in range(rmax):
                    row_vals = []
                    for c in range(cmax):
        ...
        if ext in (".xlsx", ".xlsm"):
            return await asyncio.to_thread(_xlsx)
        return await asyncio.to_thread(_xls)
