...
class RAGAnythingPipeline:
    ...
    def _get_rag_instance(self, kb_name: str, *, purpose: str):
        """Get or create RAGAnything instance."""
        kb_dir = Path(self.kb_base_dir) / kb_name
        cache_key = f"{working_dir}::{(purpose or '').strip().lower() or 'search'}"
        ...
        rag = RAGAnything(
            ...
            embedding_func=embed_client.get_document_embedding_func()
            if purpose == "index"
            else embed_client.get_query_embedding_func(),
        )

        self._instances[cache_key] = rag
        return rag
    ...
    async def initialize(
        ...
        **kwargs,
        ...
        with LightRAGLogContext(scene="knowledge_init"):
            rag = self._get_rag_instance(kb_name, purpose="index")
    ...
    async def search(
        ...
        **kwargs,
        ...
        with LightRAGLogContext(scene="rag_search"):
            rag = self._get_rag_instance(kb_name, purpose="search")
            await rag._ensure_lightrag_initialized()
