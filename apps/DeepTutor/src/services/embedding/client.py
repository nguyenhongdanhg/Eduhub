...
class EmbeddingClient:
    ...
    def __init__(self, config: Optional[EmbeddingConfig] = None):
        ...
        try:
            ...
            if self.config.binding == "sentence_transformers":
                info = {}
                try:
                ...
                try:
                    dims_int = int(dims) if dims is not None else 0
                except Exception:
                    dims_int = 0
                if dims_int > 0:
                    self.config.dim = dims_int
    ...
    async def embed(self, texts: List[str], *, input_type: Optional[str] = None) -> List[List[float]]:
        ...
        request = EmbeddingRequest(
            ...
            model=self.config.model,
            dimensions=self.config.dim,
            input_type=input_type if input_type is not None else self.config.input_type,
        )
    ...
    def embed_sync(self, texts: List[str], *, input_type: Optional[str] = None) -> List[List[float]]:
        """
        ...
        try:
            ...
            if loop.is_running():
                ...
                with concurrent.futures.ThreadPoolExecutor() as executor:
                    future = executor.submit(asyncio.run, self.embed(texts, input_type=input_type))
                    return future.result()
            else:
                return loop.run_until_complete(self.embed(texts, input_type=input_type))
        except RuntimeError:
            return asyncio.run(self.embed(texts, input_type=input_type))

    def get_embedding_func(self, *, mode: str = "auto"):
        """
        ...
        async def embedding_wrapper(texts: List[str]):
            if mode == "query":
                embeddings = await self.embed_queries(texts)
            elif mode == "document":
    ...
    def get_query_embedding_func(self):
        return self.get_embedding_func(mode="query")

    def get_document_embedding_func(self):
        return self.get_embedding_func(mode="document")
