...
class DocumentAdder:
    ...
    async def process_new_documents(self, new_files: List[Path]):
        ...
        logger.info(
            ...
            f"{len(getattr(classification, 'spreadsheet_files', []) or [])} spreadsheets, "
            f"{len(classification.unsupported)} unsupported"
        )

        processed_files = []
        spreadsheet_files = list(getattr(classification, "spreadsheet_files", []) or [])
        total_files = len(classification.needs_mineru) + len(classification.text_files) + len(spreadsheet_files)
        idx = 0
        ...
        for doc_file_str in spreadsheet_files:
            doc_file = Path(doc_file_str)
            idx += 1
            try:
                if self.progress_tracker and ProgressStage:
                    self.progress_tracker.update(
                        ProgressStage.PROCESSING_FILE,
                        f"Ingesting (spreadsheet) {doc_file.name}",
                        current=idx,
                        total=total_files,
                    )
                ...
                content = await FileTypeRouter.read_spreadsheet_file(str(doc_file))
                if content.strip():
                    await rag.lightrag.ainsert(content)
                    processed_files.append(doc_file)
                    self._record_successful_hash(doc_file)
                    logger.info(f"  ✓ Processed (spreadsheet): {doc_file.name}")
                else:
                    logger.warning(f"  ⚠ Skipped empty spreadsheet: {doc_file.name}")
            except Exception as e:
                logger.exception(f"  ✗ Failed {doc_file.name}: {e}")
